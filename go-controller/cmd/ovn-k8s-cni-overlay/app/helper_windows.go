...
// windowsVersion2016 - This is the latest windows version which does not support
// multiple containers per pod. The CNI will act differently on this version
const (
	noNameNetNS        = "none"
	containerNetNS     = "container:"
	windowsVersion2016 = 14393
)
...
func ConfigureInterface(args *skel.CmdArgs, namespace string, podName string, macAddress string, ipAddress string, gatewayIP string) ([]*current.Interface, error) {
	...
	if err != nil {
		...
		// macAddress for the endpoint should have xx-xx-xx-xx-xx-xx format
		macAddressEndpoint := strings.Replace(macAddress, ":", "-", -1)
		hnsEndpoint := &hcsshim.HNSEndpoint{
			Name:           endpointName,
			VirtualNetwork: hnsNetworkId,
			IPAddress:      ipAddr,
			MacAddress:     macAddressEndpoint,
...
func PlatformSpecificCleanup(args *skel.CmdArgs, argsMap map[string]string) error {
	// argsMap is used only for Windows Server 2016 build number 14393
	endpointName := args.ContainerID
	windowsVersion, err := getWindowsVersion()
	if err == nil && windowsVersion <= windowsVersion2016 {
	...
	if windowsVersion <= windowsVersion2016 {
		// On Windows Server 2016 we don't have to take care of the HNS Endpoint
		return nil
	}
...
// do and the CNI should stop here for Windows Server version greated than windowsVersion2016.
// For Windows Server version windowsVersion2016 it will configure the pod interface.
func InitialPlatformCheck(args *skel.CmdArgs, argsMap map[string]string) (bool, *current.Result, error) {
	...
	if err == nil && windowsVersion <= windowsVersion2016 {
		...
		if strings.Compare(args.Netns, noNameNetNS) != 0 && !checkContainerOVSPort(ifaceID) {
			result, err := configureInterface2016(args, argsMap)
			return true, result, err
		} else {
			logrus.Infof("InitialPlatformCheck: CNI has nothing to do (called for infra container or network has been setup")
	...
	if strings.HasPrefix(args.Netns, containerNetNS) || strings.Compare(args.Netns, noNameNetNS) != 0 {
		// If it is a normal container from the pod, there is nothing to do.
		// Also, if it is not an infra container, nothing to do in this case as well.
		logrus.Infof("InitialPlatformCheck: CNI has nothing to do (the container is not the infra container)")
...
// Check if the OVS port for the container interface has been already set up
...
func getWindowsVersion() (int, error) {
	...
	windowsVersion, err := strconv.Atoi(buildInfo[1])
	if err != nil {
		return 0, err
	}
...
func runWMIC(namespace string, class string, whereClause string, propertyToRetrieve string) (string, error) {
	...
	logrus.Infof("Running wmic query: 'wmic %v'", query)
...
func getContainerInterfaceName(containerID string) (string, error) {
	...
	return containerInterfaceName, nil
}

func getContainerPortInfo(containerInterfaceName string) (string, string, string, error) {
	macAddress, err := runWMIC("root\\virtualization\\v2", "Msvm_LanEndpoint",
		fmt.Sprintf("ElementName='%s' and MACAddress is not null", containerInterfaceName),
		"MACAddress")
	if err != nil {
		return "", "", "", err
	}
	...
	logrus.Infof("Found MAC Address %v for port %v", macAddress, containerInterfaceName)
	...
	if len(ipAddress) == 0 {
		return "", "", "", fmt.Errorf("Failed to find the IP for port: %v", containerInterfaceName)
	}
	logrus.Infof("Found IP %v for port: %v", ipAddress, containerInterfaceName)
	// Return the macAddress in xx:xx:xx:xx:xx:xx format
	macAddress = strings.Replace(macAddress, "-", ":", -1)
	macAddress = strings.ToLower(macAddress)
	return ipAddress, macAddress, gatewayIP, nil
}

func configureInterface2016(args *skel.CmdArgs, argsMap map[string]string) (*current.Result, error) {
	logrus.Infof("Setting up pod network on Windows Server 2016")
	containerInterfaceName, err := getContainerInterfaceName(args.ContainerID)
	if err != nil {
	...
	podName := argsMap["K8S_POD_NAME"]
	ifaceID := fmt.Sprintf("%s_%s", namespace, podName)
	ipAddress, macAddress, gatewayIP, err := getContainerPortInfo(containerInterfaceName)
	if err != nil {
	...
	// Since Windows Server 2016 doesn't support custom IP/Mac address, we have to update OVN with the given IP/Mac
	...
	logrus.Infof("Finished setting up network for container %v", args.ContainerID)
